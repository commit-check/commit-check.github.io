...
find_wheel_asset() {
    ...
    if command_exists jq; then
        ...
        # First try exact platform match, then try fallback platforms
        local wheel_info
        wheel_info=$(echo "$json_data" | jq -r --arg tool "$tool" --arg tool_us "$tool_underscore" --arg platform "$platform_tag" --arg version "$version" '
            .assets[] |
        ...
        # If exact match failed, try fallback platforms for Linux
        if [[ -z "$wheel_info" ]] && [[ "$platform_tag" == *"manylinux"* ]]; then
            # Extract architecture from platform tag
            local arch=$(echo "$platform_tag" | grep -o 'x86_64\|aarch64\|i686\|ppc64le\|s390x\|armv7l' | head -n1)
            if [[ -n "$arch" ]]; then
                # Try different manylinux versions for the same architecture
                local fallback_platforms=(
                    "manylinux_2_17_${arch}.manylinux2014_${arch}"
                    "manylinux_2_12_${arch}.manylinux2010_${arch}"
                    "manylinux_2_5_${arch}.manylinux1_${arch}"
                    "manylinux_2_27_${arch}.manylinux_2_28_${arch}"
                    "manylinux_2_26_${arch}.manylinux_2_28_${arch}"
                )
                
                for fallback in "${fallback_platforms[@]}"; do
                    wheel_info=$(echo "$json_data" | jq -r --arg tool "$tool" --arg tool_us "$tool_underscore" --arg platform "$fallback" --arg version "$version" '
                        .assets[] |
                        select(.name | test("\\.(whl)$")) |
                        select(.name | test("^(" + $tool + "|" + $tool_us + ")-")) |
                        select(.name | contains($platform)) |
                    ...
                    if [[ -n "$wheel_info" ]]; then
                        warn "Exact platform match not found, using compatible wheel: $fallback"
                        break
                    fi
                done
            fi
        ...
        echo "$json_data" | grep -o '"browser_download_url":"[^"]*"' | sed 's/"browser_download_url":"//; s/"//' | while read -r url; do
            ...
            if [[ "$filename" == *".whl" ]] && [[ "$filename" == "$tool"* || "$filename" == "$tool_underscore"* ]]; then
                # Try exact platform match first
                if [[ "$filename" == *"$platform_tag"* ]]; then
